...
function module:Notify(props)
	...
	tweenService
		:Create(
			notif.Shape,
			info,
			{ Size = UDim2.fromOffset(notif.Notification.AbsoluteSize.X, notif.Notification.AbsoluteSize.Y) }
		)
		:Play()
	...
	tweenService
		:Create(notif.Notification.Texts, info, { GroupTransparency = (configs and configs[3]) or default[3] })
		:Play()
	tweenService
		:Create(notif.Notification.Icon, info, { ImageTransparency = (configs and configs[3]) or default[3] })
		:Play()
